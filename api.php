...
switch ($action) {
    ...
    case 'update-event-settings':
        if ($method == 'POST') {
            ...
            $timer_duration = intval($input['timer_duration'] ?? 3600);
            ...
            if ($timer_duration <= 0) {
                json_response(['success' => false, 'error' => 'Длительность таймера должна быть положительной']);
            }
            ...
            try {
                ...
                json_response(['success' => false, 'error' => 'Ошибка обновления настроек: ' . $e->getMessage()]);
            }
        }
        break;
            ...
            try {
                ...
                if (!$state) {
                    ...
                    $stmt = $pdo->prepare("
                        ...
                        (id, event_name, event_status, timer_duration, is_accepting_answers) 
                        VALUES (1, 'Математическая мясорубка', 'not_started', 3600, 0)
                    ");
                    $stmt->execute();
                    $stmt = $pdo->prepare("SELECT * FROM event_state WHERE id = 1");
                    $stmt->execute();
                ...
                if ($state['event_status'] === 'running' && $state['event_end_time']) {
                    // ИСПРАВЛЕННЫЙ ЗАПРОС: используем другой алиас вместо current_time
                    $stmt = $pdo->query("SELECT NOW() as current_db_time, UNIX_TIMESTAMP() as unix_now");
                    $time_info = $stmt->fetch();
                ...
                $response_state = array_merge($default_state, $state);
                
                json_response($response_state);
                
            } catch (PDOException $e) {
                error_log("Get event state full error: " . $e->getMessage());
                json_response([
                    'event_name' => 'Математическая мясорубка',
                    'event_status' => 'not_started', 
                    'timer_duration' => 3600,
                    ...
                    'is_ranking_frozen' => false,
                    'is_accepting_answers' => false,
                    'error' => 'Ошибка получения состояния мероприятия: ' . $e->getMessage()
                ]);
            }
        }
        break;
